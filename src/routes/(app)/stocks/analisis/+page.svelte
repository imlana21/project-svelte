...
{#snippet cell(item: StockAnalysis, column: ColumnDef)}
	...
	{:else if column.key === "is_valid"}
		<span
			class="badge {item.is_valid
				? 'bg-success-500 text-success-contrast-500'
				: 'bg-surface-500 text-surface-contrast-500'}"
		>
...
<CrudPage
	...
	{cell}
	canDetail={can(PERMISSIONS.stocksAnalyses.read)}
	canEdit={can(PERMISSIONS.stocksAnalyses.update)}
	canDelete={can(PERMISSIONS.stocksAnalyses.delete)}
