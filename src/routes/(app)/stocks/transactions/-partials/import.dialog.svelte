<script lang="ts">
	...
	function validate(
		...
		if (missingRequired.length > 0) {
			globalErrors = [
				`Kolom wajib tidak ditemukan: ${missingRequired.join(", ")}. Kolom tersedia: ${headers.join(", ")}`,
			];
			return {};
		}
		globalErrors = [];
		...
		for (let i = 0; i < rows.length; i++) {
			...
			if (!type) rowErrors.type = "Wajib diisi";
				...
				rowErrors.type = "Harus buy atau sell";

			if (!row.date?.trim()) rowErrors.date = "Wajib diisi";
			else if (!validateDate(row.date.trim()))
				rowErrors.date = "Format harus YYYY-MM-DD";
	...
	const validRowCount = $derived(
		parsedRows.length - Object.keys(validationErrors).length,
	);
	...
	{#if step === "pick"}
		...
		>
			...
			<p class="text-sm text-surface-600 dark:text-surface-400">
				Seret file CSV ke sini atau <span class="text-primary-500 font-medium"
		...
		<div class="mt-4 rounded-lg bg-surface-100 p-3 text-xs dark:bg-surface-800">
			...
			<ul class="mt-1 space-y-0.5 text-surface-500 dark:text-surface-400">
				...
				<li>
					<strong>price</strong> (wajib) - Harga per lembar (angka &gt; 0)
				</li>
	...
	{:else if step === "preview"}
		...
		<div class="mb-3 flex items-center gap-4 text-xs">
			...
			<span class="text-surface-500"
				>Total baris: <strong>{parsedRows.length}</strong></span
			>
			{#if validRowCount > 0}
		...
		<div
			class="max-h-64 overflow-auto rounded-lg border border-surface-200 dark:border-surface-700"
		>
					...
					{#each parsedRows as row, i}
						...
						<tr
							class="border-t border-surface-100 dark:border-surface-800 {hasRowError
								? 'bg-error-50/50 dark:bg-error-950/30'
								: ''}"
						>
							...
							{#each parsedHeaders as header}
								...
								>
									{row[header] || "-"}
									{#if cellError}
										<span class="block text-[10px] text-error-500"
											>{cellError}</span
										>
									{/if}
								</td>
	...
	{:else if step === "result"}
		<div class="flex flex-col items-center gap-3 py-4 text-center">
			...
			{#if result?.errors && result.errors.length > 0}
				...
				>
					<p class="font-medium text-error-700 dark:text-error-300">
						Detail error:
					</p>
...
{#snippet footerSnippet()}
	{#if step === "pick"}
		<button type="button" class="btn" onclick={() => handleOpenChange(false)}
			>Batal</button
		>
	{:else if step === "preview"}
		<button type="button" class="btn" onclick={() => handleOpenChange(false)}
