<script lang="ts">
	...
	function handleSubmit(e: SubmitEvent) {
		...
		if (!validate()) return;
		if (item) {
			onSubmit({ source: source.trim() || undefined, note: note.trim() || undefined } as IncomeForm);
		} else {
			onSubmit({
				amount,
...
>
	<form id="income-form" class="flex flex-col gap-4" onsubmit={handleSubmit}>
		...
		{/if}
		<Field label="Sumber">
			<input class="input" type="text" placeholder="Gaji, Bonus, dll." bind:value={source} />
		</Field>
		<Field label="Catatan">
...
{#snippet footerSnippet()}
	...
	<button type="submit" form="income-form" class="btn bg-primary-500 text-primary-contrast-500" disabled={saving}>
